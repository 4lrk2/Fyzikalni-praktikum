...
if __name__ == "__main__":
    ...
    parser.add_argument("--fit-std-threshold", type=float, default=1000,
    ...
    parser.add_argument("-f", "--min-fit-frames", type=int, default=2,
    ...
    parser.add_argument("-x", "--x-std-threshold", type=float, default=10000,
    ...
    for video_file in video_files:
        ...
        plt.ylabel("Y [px]")
        if save_plots:
            plt.savefig(f"{video_file}_plot.jpg")
            sleep(0.5)
        plt.show(block=False)
        if speed > 0:
            plt.pause(speed)
